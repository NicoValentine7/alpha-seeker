...
2. データ取得の非同期化（asyncio + aiohttp）でS&P500全社取得を高速化
3. インサイダー取引データ（SEC EDGAR Form 4）
4. ニュースセンチメント分析（FinBERT）
5. Moomoo証券API連携（ポートフォリオ取得・自動取引）
6. 定期実行スクリプト（cron）によるデータ自動収集
