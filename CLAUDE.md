...
`--extra`で追加された保有銘柄は以下の安全策で保護される:
- **セクター情報欠損時の保護**: yfinanceからセクター情報が取得できなかった場合、通常の銘柄は除外されるが、`--extra`銘柄は`"Unknown"`セクターとして保持される
- **ranking.json生成時の警告**: CIでranking.json生成時、`portfolio_tickers`に含まれる銘柄がCSVに存在しない場合は警告ログを出力

### CSV保存時のnumpy型変換

CSV保存時、リスト/辞書型カラム（`earnings_surprises`等）内の`np.float64`等のnumpy型をPython標準型に変換してから文字列化する。これにより`ast.literal_eval`で正しく復元でき、ranking.jsonに詳細データが含まれるようになる。
