...
.th-tip {{ display: none; position: absolute; top: calc(100% + 8px); left: 50%; transform: translateX(-50%); z-index: 100; background: #1c2128; border: 1px solid #30363d; border-radius: 8px; padding: 16px 20px; min-width: 320px; max-width: 400px; box-shadow: 0 12px 32px rgba(0,0,0,0.6); white-space: normal; text-transform: none; letter-spacing: normal; font-weight: 400; line-height: 1.7; }}
thead th:hover .th-tip {{ display: block; }}
.th-tip .tip-title {{ font-weight: 700; font-size: 14px; color: #f0f6fc; margin-bottom: 10px; padding-bottom: 8px; border-bottom: 1px solid #30363d; }}
.th-tip .tip-simple {{ font-size: 13px; color: #e6edf3; margin-bottom: 12px; line-height: 1.8; }}
.th-tip .tip-detail {{ font-size: 12px; color: #8b949e; border-top: 1px solid #30363d; padding-top: 10px; line-height: 1.8; }}
...
<thead><tr>
  ...
  <th data-key="total" data-type="num">総合<span class="th-tip"><span class="tip-title">総合スコア (0-100)</span><span class="tip-simple">この株がどれだけ「伸び代のある過小評価株」かを示す総合評価</span><span class="tip-detail">割安度(25%) + 成長力(30%) + 質(20%) + 決算勢い(25%)の加重平均。セクター内の相対比較。70+は有望、60+は注目、45未満は割高。バリュートラップ銘柄は-20点のペナルティ。</span></span></th>
  <th data-key="val" data-type="num">割安度<span class="th-tip"><span class="tip-title">割安度スコア (0-100)</span><span class="tip-simple">同じセクターの他社と比べて、この株がどれだけ安いか</span><span class="tip-detail">PER(20%), PBR(15%), EV/EBITDA(20%), PSR(15%), FCF利回り(30%)をセクター内でパーセンタイルランク化。数値が高いほどセクター内で相対的に割安。FCF利回りの重みが最大（30年バックテストで最高リターンの実績）。</span></span></th>
  <th data-key="grw" data-type="num">成長力<span class="th-tip"><span class="tip-title">成長力スコア (0-100)</span><span class="tip-simple">この会社がどれだけ速く成長しているか</span><span class="tip-detail">売上成長率(30%), 営業利益成長率(25%), EPS成長率(30%), PEGレシオ(15%)で評価。PEGは「成長速度に対して株価が安いか」を測る指標（1.0未満が割安成長）。</span></span></th>
  <th data-key="qlt" data-type="num">質<span class="th-tip"><span class="tip-title">質・健全性スコア (0-100)</span><span class="tip-simple">経営の質と財務の健全性を示す</span><span class="tip-detail">ROE(30%), 粗利率(20%), D/Eレシオ(25%), FCFマージン(25%)で評価。ROEが高い＝株主資本を効率的に使っている。粗利率が高い＝競争優位性がある。D/Eが低い＝借金が少ない。FCFマージンが高い＝実際にキャッシュを生んでいる。</span></span></th>
  <th data-key="em" data-type="num">決算勢い<span class="th-tip"><span class="tip-title">決算モメンタムスコア (0-100)</span><span class="tip-simple">決算発表で市場の予想を上回り続けているか</span><span class="tip-detail">決算サプライズ率(25%), アナリストEPS予想の上方修正(25%), 売上成長の加速度(20%), 来期EPS成長予想(30%)で評価。アナリスト予想を連続で上回る銘柄は、まだ市場に正しく評価されていない可能性が高い。</span></span></th>
  <th data-key="fscore" data-type="num">F値<span class="th-tip"><span class="tip-title">Piotroski F-Score (0-9)</span><span class="tip-simple">財務の健全さを9項目でチェックした点数</span><span class="tip-detail">収益性(ROA, 営業CF, ROA改善, CF&gt;利益)で4点、レバレッジ(負債低下, 流動比率改善, 希薄化なし)で3点、効率(粗利率改善, 資産回転率改善)で2点。7+は優良、4-6は普通、0-3は懸念。F-Score 2以下はバリュートラップとしてペナルティ。</span></span></th>
  <th data-key="pe" data-type="num">PER<span class="th-tip"><span class="tip-title">PER (株価収益率)</span><span class="tip-simple">株価が利益の何倍か。低いほど割安</span><span class="tip-detail">株価 / 1株あたり利益(EPS)。15以下は割安、20-25は適正、30以上は割高が目安。ただしセクターによって水準が異なるため、同業他社との比較が重要。</span></span></th>
  <th data-key="fpe" data-type="num">予想PE<span class="th-tip"><span class="tip-title">予想PER (Forward PE)</span><span class="tip-simple">来期の予想利益に対する株価の倍率</span><span class="tip-detail">株価 / 来期予想EPS。現在のPERより低ければ、利益成長が期待されている証拠。PERとの差が大きいほど成長期待が高い。</span></span></th>
  <th data-key="peg" data-type="num">PEG<span class="th-tip"><span class="tip-title">PEGレシオ</span><span class="tip-simple">成長速度に対して株価が安いか高いかを測る</span><span class="tip-detail">PER / EPS成長率(%)。1.0以下なら成長に対して割安（GARP基準）、0.5以下は非常に割安。Peter Lynchが普及させた指標。ただし赤字企業や低成長企業には使えない。</span></span></th>
  <th data-key="fcfy" data-type="num">FCF利回<span class="th-tip"><span class="tip-title">FCF利回り (%)</span><span class="tip-simple">企業が生み出す余剰キャッシュの利回り</span><span class="tip-detail">フリーキャッシュフロー / 企業価値(EV)。PERより操作されにくく、30年バックテストで最高リターンを記録した指標。7%以上は魅力的、マイナスは要注意。</span></span></th>
  <th data-key="roe" data-type="num">ROE<span class="th-tip"><span class="tip-title">ROE - 自己資本利益率 (%)</span><span class="tip-simple">株主のお金をどれだけ効率よく利益に変えているか</span><span class="tip-detail">当期純利益 / 自己資本。15%以上は優秀、20%以上は非常に高い。ただし借入を増やしてもROEは上がるため、D/Eと合わせて評価する必要がある。</span></span></th>
  <th data-key="gm" data-type="num">粗利率<span class="th-tip"><span class="tip-title">粗利率 (Gross Margin %)</span><span class="tip-simple">売上からモノの原価を引いた利益の割合</span><span class="tip-detail">(売上 - 売上原価) / 売上。高いほど価格決定力や競争優位性がある。ソフトウェア企業は70-80%、製造業は30-40%が目安。Novy-Marx(2013)の研究でリターン予測力が確認された指標。</span></span></th>
  <th data-key="de" data-type="num">D/E<span class="th-tip"><span class="tip-title">D/Eレシオ (負債/自己資本)</span><span class="tip-simple">借金の多さ。低いほど安全</span><span class="tip-detail">有利子負債 / 自己資本(%)。100以下は健全、200以上は高め、500以上はバリュートラップのリスク。自社株買いで自己資本がマイナスの優良企業（HD, KO等）は異常に高くなることがある。</span></span></th>
  <th data-key="revg" data-type="num">売上成長<span class="th-tip"><span class="tip-title">売上成長率 (前年比 %)</span><span class="tip-simple">売上がどれだけ伸びたか</span><span class="tip-detail">年次の売上高の前年比成長率。10%以上は高成長、20%以上は急成長。長期的には売上成長がEPS成長の源泉になるため、最も重要な成長指標の一つ。</span></span></th>
  <th data-key="epsg" data-type="num">EPS成長<span class="th-tip"><span class="tip-title">EPS成長率 (前年比 %)</span><span class="tip-simple">1株あたり利益がどれだけ伸びたか</span><span class="tip-detail">純利益ベースのEPS前年比成長率。株価は長期的にEPS成長に連動する。自社株買いでもEPSは成長するため、売上成長と合わせて評価する。</span></span></th>
  <th data-key="surp" data-type="num">決算サプライズ<span class="th-tip"><span class="tip-title">平均決算サプライズ率 (%)</span><span class="tip-simple">アナリスト予想を実績がどれだけ上回ったか</span><span class="tip-detail">直近4四半期のEPSサプライズ率の平均。プラスが連続する銘柄は「アナリストがまだ追いついていない」＝過小評価のサイン。PEAD（決算後ドリフト）効果が実証されている。</span></span></th>
  <th data-key="epsrev" data-type="num">EPS修正<span class="th-tip"><span class="tip-title">EPS予想修正率 (90日間 %)</span><span class="tip-simple">アナリストが90日間でEPS予想をどれだけ引き上げたか</span><span class="tip-detail">90日前のEPS予想と現在の予想の変化率。上方修正が続く銘柄は株価が上昇しやすい。Zacksの研究で予測力が最も高いファクターの一つと確認されている。</span></span></th>
  <th data-key="price" data-type="num">株価</th>
  <th data-key="upside" data-type="num">上昇余地<span class="th-tip"><span class="tip-title">上昇余地 (%)</span><span class="tip-simple">アナリストの目標株価まであとどれだけ上がる余地があるか</span><span class="tip-detail">(アナリスト目標株価の平均 - 現在株価) / 現在株価。20%以上は十分な上昇余地。ただしアナリスト予想は楽観的な傾向がある点に注意。</span></span></th>
  <th data-key="rec" data-type="str">推奨<span class="th-tip"><span class="tip-title">アナリスト推奨</span><span class="tip-simple">ウォール街のアナリストの総合評価</span><span class="tip-detail">buy(買い), strong_buy(強い買い), hold(保持), sell(売り)。複数アナリストの評価のコンセンサス。</span></span></th>
  <th data-key="beat" data-type="str">ビート<span class="th-tip"><span class="tip-title">決算ビート率</span><span class="tip-simple">直近4四半期で何回アナリスト予想を上回ったか</span><span class="tip-detail">例: 4/4 = 4四半期連続でEPS予想を上回った。3/4以上は強い。連続ビートは市場がまだこの会社の実力を正しく評価していない可能性を示す。</span></span></th>
</tr></thead>
